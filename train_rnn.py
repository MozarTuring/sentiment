...
if NUM_ELE == 26:
  weights = get_weights(train_iter, label_field)
else:
...
model = create_model(args, vocab_ls, label_field, USE_GPU, DEVICE, NUM_ELE, logger)
if pre_embedding:
...
train_start(args.EPOCHS, model, train_iter, dev_iter, loss_functions,
            optimizer, writer, logger, out_dir, label_field, DEVICE, GLOBAL_STEP)
