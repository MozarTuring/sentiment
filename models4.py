...
class BiLSTMSentiment(nn.Module):
  def __init__(self, embedding_dim, hidden_dim, vocab_size,
    ...
    self.fcs2 = nn.ModuleList([nn.Linear(hidden_dim, 4), nn.Linear(hidden_dim, 3)])

  def forward(self, sentence):
    x = self.embeddings(sentence)
    lstm_out, self.hidden = self.lstm(torch.transpose(x, 0, 1))
    attn_weights = self.attn(self.task_vecs, lstm_out)
    context = torch.matmul(attn_weights.transpose(1, 2), lstm_out.transpose(0, 1)) # 26,64,1,200
    ...
    return (log_prob1, log_prob2, attn_weights)